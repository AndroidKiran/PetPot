...
import com.droid47.petpot.search.presentation.ui.widgets.SortPopupMenu
...
class FilterFragment :
    ...
    override fun getClassName(): String = FilterFragment::class.java.simpleName

    override fun getScreenName(): String = AnalyticsScreens.FILTER_SCREEN
    ...
    private fun updateCategory() {
        ...
        val title = getViewDataBinding().navigationMenuView.checkedItem?.title?.toString()
            ?.toLowerCase(Locale.US) ?: return
        getViewModel().run {
            trackFilterChange(title)
            categoryLiveData.postValue(title)
        }
    }
